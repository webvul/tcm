...
function BaseLayerView(id, clz){
	...
	return {
		...
		getBodyContainer : function(){ 
			...
			return $XH.first(_checkPanel(), "ixw-body");
		},
...
IXW.Lib.ModalDialog = function(cfg){
	...
	function _resize(){
		...
		if (posY < 120) 
			return bodyEl.style.marginTop = -$Xw.getScreen().size[1] + 120 + "px";
...
function showErrMsg(err){
	NV.Dialog.alert(err || "请输入正确的页码数字。");
}
...
function pgGo(params, aEl){
	...
	if (idx==getCurPageIdx(pgEl))
		return NV.Dialog.alert("当前已经是第" + idx + "页，请重新输入。");
